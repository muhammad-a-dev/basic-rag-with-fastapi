...
def _unique_upload_path(temp_dir: Path, safe_name: str) -> Path:
    """Place uploads under a unique prefix so same-name files do not overwrite."""
    return temp_dir / f"{uuid.uuid4().hex[:12]}_{safe_name}"


def _header_safe_sources(sources: list[str]) -> str:
    """Join source labels into an HTTP header value without CR/LF or controls."""
    cleaned: list[str] = []
    for source in sources:
        label = _HEADER_UNSAFE.sub("", source).replace(",", " ").strip()
        if label:
...
def _unlink_quiet(path: Path) -> None:
    """Best-effort delete for temp uploads; never raise to the client."""
    try:
        path.unlink(missing_ok=True)
    except OSError:
        logger.warning("Could not remove temp upload %s", path, exc_info=True)
...
def query(request: QueryRequest) -> StreamingResponse | EmptyRetrievalResponse:
    ...
    return StreamingResponse(
        ...
        headers={"X-Sources": _header_safe_sources(result.sources)},
    )
